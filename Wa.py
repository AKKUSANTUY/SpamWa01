...
# -----------------------------------------------------------
# Tidak ada author Untuk Sc ini kecuali SANTRI CYBER SANTUY yg Telah Mendesign Dengan Sempurna
...
def files():
	...
	if fil in os.listdir(os.getcwd()):
		...
		js=int(input(k+'\tTotal spam cuk: '+h))
...
def single():
	nomer=str(input(k+'\tPhone Number Cuk : '+h))
	jm=int(input(k+'\tTotal Spam Cuk : '+h))
	dly=int(input(k+'\tDelay Cuk : '+h))
	for oo in range(jm):
		z=spam(nomer)
		if jns == 'ktbs':
			print('\t'+z.spam())
...
def multi():
	nomer=[]
	jum=int(input(k+'\tTotal Number Cuk : '+h))
	for i in range(jum):
		nomer.append(str(input(k+f'\tNumber -{i+1} : '+h)))
	spm=int(input(k+'\tTotal Spam Cuk : '+h))
	dly=int(input(k+'Delay : '+h))
	kk=len(nomer)
	for i in range(spm):
		for ss in range(kk):
...
def logo():
	os.system('clear')
	auth=m+'  Author : '+k+'SANTRI CYBER SANTUY'
...
%s┃┈┈┈┣▅╋▅┫┃%s╲╲╲╲╲╲%s╚═╗╠═╝╠═╣║║║
%s┃┈┃┈╰━╰━━━━━━╮%s╲╲%s╚═╝╩  ╩ ╩╩ ╩
%s╰┳╯┈┈┈┈┈┈┈┈┈◢▉◣%s╲%s╔═╗╔╦╗╔═╗ WhatsApp  : 082134566596
%s╲┃┈┈┈┈┈┈┈┈┈┈▉▉▉%s╲%s╚═╗║║║╚═╗ Instagram : Akku_Santuy
%s╲┃┈┈┈┈┈┈┈┈┈┈◥▉◤%s╲%s╚═╝╩ ╩╚═╝ Message   : Keep It Santuy
%s╲┃┈┈┈┈╭━┳━━━━╯%s╲╲%s╦ ╦╦ ╦╔═╗╔╦╗╔═╗╔═╗╔═╗╔═╗
%s╲┣━━━━━━┫%s╲╲╲╲╲╲╲%s║║║╠═╣╠═╣ ║ ╚═╗╠═╣╠═╝╠═╝
%s╲┃┈┈┈┈┈┈┃%s╲╲╲╲╲╲╲%s╚╩╝╩ ╩╩ ╩ ╩ ╚═╝╩ ╩╩  ╩  
...
def termux():
	...
	for w in range(int(input(u+'\tTotal spam Cuk : '+h))):
		z=spam(nj)
		if jns == 'ktbs':
			print('\t'+z.spam())
		elif jns == 'tkpd':
...
def main():
	...
	pil=str(input(b+'╚══'+m+'〙'+u+'Mode Cuk'+m+' ▶ '+h))
	if( pil == '1' or pil == '01'):
		single()
